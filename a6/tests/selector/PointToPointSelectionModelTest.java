...
class PointToPointSelectionModelTest {
    ...
    @DisplayName("GIVEN a model with a non-empty selection path, WHEN liveWire is called, "
            + "THEN it should return a line segment from the last selected point to the "
            + "mouse position.")
    @Test
    void testNonEmptyLiveWire(){
        // Set up the test scenario
        SelectionModel model = new PointToPointSelectionModel(false);
        Point startPoint = new Point(0, 0);
        model.addPoint(startPoint);
        Point endPoint = new Point (1, 1);
        model.addPoint(endPoint);

        // Perform the test action
        Point mouseLocation = new Point(2, 2);
        PolyLine wire = model.liveWire(mouseLocation);

        // Verify the consequences
        PolyLine expectedWire = new PolyLine(endPoint, mouseLocation);
        assertEquals(expectedWire, wire);
    }
    @DisplayName("GIVEN a model with a non-empty selection path that is not finished, "
            + "WHEN undo() is called, THEN the last point added should be removed "
            + "from the model, ending at its previous penultimate point, "
            + "AND listeners will be notified that its selection has changed, AND listeners"
            + "will not be notified that a state has changed(because it hasn't)")
    @Test
    void testNonEmptyUndo(){
        // Set up the test scenario
        SelectionModel model = new PointToPointSelectionModel(false);
        Point startPoint = new Point(0, 0);
        model.addPoint(startPoint);
        Point midPoint = new Point (1, 1);
        model.addPoint(midPoint);
        ...
        observer.assertChanged("selection");
        assertEquals(1, model.selection().size());
        assertEquals(new Point(1, 1), model.lastPoint());
    ...
    @DisplayName("GIVEN a selection, WHEN a point at the start of the selection path is moved, "
            + "THEN the first segment's start and last segment's end will have their start or end moved to "
            + "the new location as appropriate.")
    @Test
    void testMovePointStart() {
        ...
        observer.assertChanged("selection");
        PolyLine beforeSegment = model.selection().getLast(); //now these are the first and last segments
        PolyLine afterSegment = model.selection().getFirst();
        assertEquals(new Point(-5, 5), beforeSegment.end());
        assertEquals(new Point(-5, 5), afterSegment.start());
    }

    @DisplayName("GIVEN a selection, WHEN a point at the end of the selection path is moved, "
            + "THEN the last segment's start and the penultimate segment's end will have their start "
            + "or end moved to the new location as appropriate.")
    @Test
    void testMovePointEnd() {
        SelectionModel model = makeSquareSelection();
        PclTester observer = new PclTester();
        model.addPropertyChangeListener(observer);

        model.movePoint(model.selection().size()-1, new Point(-5, 5));
        observer.assertChanged("selection");
        PolyLine beforeSegment = model.selection().get(model.selection.size()-2); //now these are the first and last segments
        PolyLine afterSegment = model.selection().get(model.selection.size()-1);
        assertEquals(new Point(-5, 5), beforeSegment.end());
        assertEquals(new Point(-5, 5), afterSegment.start());
    }
    ...
    @DisplayName("GIVEN a selection, WHEN querying for the closest point to a location farther from "
            + "all of the control points than the max distance squared, THEN -1 will be returned.")
    @Test
    ...
    @DisplayName("GIVEN a selection (with no duplicate control points), WHEN querying for the "
            + "closest point to a location near (but not on) one of the control points and within "
            + "the max distance, THEN the index of the nearest control point will be returned.")
    @Test
    void testClosestPointWithinRange() {
        SelectionModel model = makeSquareSelection();
        assertEquals(2, model.closestPoint(new Point(10, 7), 15));
    }
}
