...
public abstract class SelectionModel {
    ...
    public Point lastPoint() {
        ...
        if (selection.isEmpty()){
            ...
            PolyLine lastLine = selection.getLast();
            return new Point(lastLine.end().x, lastLine.end().y);
        }
    }
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        if (state != SELECTED) {
            throw new IllegalStateException("Selection must be finished");
        }
        int closest = -1;
        double minDistanceSq = Double.POSITIVE_INFINITY;
        for (int i = 0; i < selection.size(); i++) {
            PolyLine segment = selection.get(i);
            Point start = segment.start();
            double distanceSq = start.distanceSq(p);
    ...
    protected void undoPoint() {
        if (selection.isEmpty()) {
            ...
            if (state == SELECTED){ //selection has finished
                setState(SELECTING); //change state back to selecting
                propSupport.firePropertyChange("state", null, state);
            }
