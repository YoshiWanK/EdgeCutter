...
public class SelectionComponent extends JComponent implements MouseListener, MouseMotionListener,
    ...
    private void paintSelectionPerimeter(Graphics g, List<PolyLine> segments) {
        ...
        for (PolyLine seg : segments) {
            g.drawPolyline(seg.xs(), seg.ys(), seg.size());
        }
    }
    ...
    private void paintLiveWire(Graphics g) {
        ...
        PolyLine poly = model.liveWire(mouseLocation);
        g.drawPolyline(poly.xs(),poly.ys(), poly.size());
    }
    ...
    private void paintControlPoints(Graphics g, List<PolyLine> segments) {
        ...
        for(PolyLine seg : segments) {
            g.fillOval(seg.xs()[0]-controlPointRadius, seg.ys()[0]-controlPointRadius,
    ...
    private void paintMoveGuides(Graphics g, List<PolyLine> segments) {
        assert(selectedIndex >= 0 && selectedIndex < segments.size());
        g.setColor(liveWireColor);

        Point prev = selectedIndex == 0 ? segments.getLast().start() : segments.get(selectedIndex-1).start();
        g.drawLine(mouseLocation.x, mouseLocation.y, prev.x, prev.y);

        Point next = selectedIndex == segments.size() - 1 ? segments.getFirst().start() : segments.get(selectedIndex).end();
        g.drawLine(mouseLocation.x, mouseLocation.y, next.x, next.y);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getButton() == MouseEvent.BUTTON1 && (model.state() == NO_SELECTION || model.state() == SELECTING)) {
            model.addPoint(mouseLocation);
        }
        else if (e.getButton() == MouseEvent.BUTTON2 && model.state() == SELECTING) {
            model.finishSelection();
        }
        else if (e.getButton() == MouseEvent.BUTTON3 && (model.state() == SELECTING || model.state() == SELECTED)) {
            model.undo();
        }
    ...
    public void mousePressed(MouseEvent e) {
        if(e.getButton() == MouseEvent.BUTTON1 && model.state() == SELECTED) {
            ...
            selectedIndex = model.closestPoint(current, maxDistanceSq);
            updateMouseLocation(e.getPoint());
        }
    }
